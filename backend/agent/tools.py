...
def create_session_tools(session_id: str):
    """
    Create session-scoped LangChain tools via closure.
    Each tool has the session_id baked in so the LLM doesn't need to know about it.
    """
    ...
    def search_events_by_semantics(query: str, top_k: int = 5):
        ...
        # Filter by session_id in Pinecone metadata
    ...
    def get_video_info():
        ...
        # Look in the session-specific upload directory
        if session_id:
        ...
        files = [f for f in os.listdir(upload_dir) if not os.path.isdir(os.path.join(upload_dir, f))]
        if not files:
    ...
    return [search_events_by_semantics, query_events_by_sql, count_events, get_video_info, analyze_video_visually]
